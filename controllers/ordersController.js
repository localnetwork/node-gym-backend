...
const addOrder = async (req, res) => {
  ...
  if (!paymentMethod && payment_method) {
    errors.push({
      payment_method:
        "Payment method is no longer available. Please select another.",
    });
  }
  ...
  if (!availedPromo && promo) {
    errors.push({
      promo: "Promo is no longer available. Please select another.",
    });
  }
...
const viewAllOrders = async (req, res) => {
  try {
    ...
    if (start_date) {
      ...
      filters.push(
        `DATE(FROM_UNIXTIME(orders.created_at / 1000)) >= '${startDate}'`
      );
    }
    if (end_date) {
      ...
      filters.push(
        `DATE(FROM_UNIXTIME(orders.created_at / 1000)) <= '${endDateStr}'`
      );
    }
    if (order_id) {
    ...
    for (const row of results) {
      ...
      if (pm !== "paypal") {
        offline_transaction = await entity.findOfflineTransactionByOrderId(
          row.id
        );
      }
...
const checkout = async (req, res) => {
  ...
  const proofUrl =
    proofFile?.destination.replace("public", "") + "/" + proofFile?.filename;
  ...
  const hasLifetime = await entity.findUserHasLifetimeSubscription(
    availed_by?.userId
  );
  if (hasLifetime) {
    return res.status(400).json({
      status_code: 400,
      message:
        "You already have a lifetime subscription. You cannot extend or buy a new promo.",
      error:
        "You already have a lifetime subscription. You cannot extend or buy a new promo.",
    });
  }
    ...
    paypal.payment.execute(
      ...
      async (error, payment) => {
        if (error) {
          ...
          const lastSubscriptionStartDate =
            lastOrderResult.length > 0
              ? lastOrderResult[0].subscription_date
              : null;
          ...
          if (!lastSubscriptionStartDate) {
            ...
            newSubscriptionStartDate = new Date(
              currentTime + durationDays * 24 * 60 * 60 * 1000
            ); // Add durationDays in milliseconds
          } else {
            newSubscriptionStartDate =
              parseInt(lastSubscriptionStartDate) +
...
const approveOrder = async (req, res) => {
  ...
  try {
    ...
    if (order.mode_payments === 2) {
      return res
        .status(400)
        .json({ error: "Paypal payment method cannot be approved manually." });
    }
    ...
    const lastSubscriptionStartDate =
      lastOrderResult.length > 0 ? lastOrderResult[0].subscription_date : null;
    ...
    return res
      .status(200)
      .json({ message: "Order approved and status updated to completed." });
  } catch (error) {
...
const rejectOrder = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Order rejected and status updated to rejected." });
  } catch (error) {
